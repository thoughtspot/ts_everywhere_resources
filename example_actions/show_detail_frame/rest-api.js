 ...
 * @param secret_key The cluster level secret key.  Available in Developer -> Security Settings.
 * @param username The name of the user to get a token for.
 * @param access_level Either "FULL" or "REPORT_BOOK_VIEW"
 * @param ids A list of IDs the user has access to if not "FULL" access_level.
 * @returns {Promise<any>}  A promise with the token if successful.
 * getTokenForUser.then(info => {
 *   const token = response.text();
 * }
 */
export const getTokenForUser = async (tsurl, secret_key, username, access_level = "FULL", ids=None) => {
  console.log(`Getting token for ${username}`);
  // TODO - add code to get the user token.
  const tokenURL = "";
...
export const loginWithToken = async (tsurl, username, auth_token, redirect_url = null) => {
  ...
  await fetch(loginURL, {
    ...
    .then(response => console.log("Logged into ThoughtSpot using token"))
    .catch(error => console.error(error));
}
